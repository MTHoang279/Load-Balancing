...
//    assign s_tready = (!buf_valid) || (m_tready);
    ...
    reg [31:0] valid_pkt_count;
    
    
//    always @(posedge clk) begin
//        if (!rst_n) begin
//            valid_pkt_count <= 32'd0;
//        end else begin
//            if (in_fire) begin
//                if (state == ST_IDLE &&
//                    s_tdata[415:400] == ETH_TYPE_IPV4 &&
//                    s_tlast) begin
...
//    end
    always @(posedge clk) begin
        if (in_fire && buf_valid && !out_fire) begin
            $display("OVERWRITE x?y ra!");
        end
    end
    /* ---------------- MAIN LOGIC -------------------- */
    always @(posedge clk) begin
