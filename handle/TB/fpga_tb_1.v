...
module tb_fpga_top;
    ...
    localparam INPUTFILE  = "E:/10G_Ethernet/UDP_sample_10K.dat";
    localparam CLK_FREQ_HZ = 1000;
    localparam TICK_MS = 10;
    localparam NUM_SERVERS = 36;
...
//            if (m_tlast_d && !dut.u_packet_filter.m_tlast && dut.u_packet_filter.m_tvalid) begin
    ...
    reg backend_tlast_d;
    always @(posedge clk_p) begin
        backend_tlast_d <= (dut.u_server.rx_user_last && dut.u_server.rx_user_valid);
        if (!rst_in) begin
            // detect falling edge: 1 -> 0
            if (backend_tlast_d && !(dut.u_server.rx_user_last && dut.u_server.rx_user_valid)) begin
...
//    // USER REQ RX
//    wire [31:0] cnt_user_req_rx   [0:NUM_SERVERS-1];
//    // HB REQ RX
//    wire [31:0] cnt_hb_req_rx     [0:NUM_SERVERS-1];
//    // USER REPLY TX
//    wire [31:0] cnt_user_reply_tx [0:NUM_SERVERS-1];
//    // HB REPLY TX
//    wire [31:0] cnt_hb_reply_tx   [0:NUM_SERVERS-1];
...
//                dut.u_server.cnt_user_req_rx[i*32 +: 32];
    
//            assign cnt_hb_req_rx[i] =
//                dut.u_server.cnt_hb_req_rx[i*32 +: 32];
    
//            assign cnt_user_reply_tx[i] =
//                dut.u_server.cnt_user_reply_tx[i*32 +: 32];
    
//            assign cnt_hb_reply_tx[i] =
//                dut.u_server.cnt_hb_reply_tx[i*32 +: 32];
...
//    end
    ...
    initial begin
        ...
        start   = 1'b0;
        algo_sel = 3'b001;   // Consistent Hash
