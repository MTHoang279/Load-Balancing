module shm_parser #(
    ...
    // sst_scn_dec_en: COMBINATORIAL - asserted same clock as handshake,
    // so sst_controller can latch it at posedge without a 1-cycle delay race.
    output wire                       sst_scn_dec_en,
    output wire [SERVER_ID_WIDTH-1:0] sst_server_idx,

    // scn_dec_en / server_idx: REGISTERED (1 cycle after handshake).
    // Dung cho CSLB de nhan biet khi nao co user response va server nao.
    output reg                        scn_dec_en,
    output reg  [SERVER_ID_WIDTH-1:0] server_idx,

    output reg                        parser_response_valid,
    output reg [31:0]                 parser_server_ip,
    ...
    assign udp_src_port = rx_user_data[319:304];
    assign udp_dst_port = rx_user_data[335:320];
    assign src_ip       = rx_user_data[271:240];
