module fpga_top#(
    ...
    parameter FILTER_UDP_IPV4_ONLY = 1
)(
    input  wire        clk_p,
    input  wire        clk_n,    
...
//    input  wire        loop_en,
    ...
    always @(*) begin
        ...
        server_en_valid = {NUM_SERVERS{1'b0}}; 

        case(server_en)
            6'b000000: server_en_valid = {NUM_SERVERS{1'b0}};                   // 0 server
            
            6'b000001: server_en_valid[0] = 1'b1;                             // B?t server 0
            
            6'b000011: begin
                if (NUM_SERVERS >= 4) server_en_valid[3:0] = 4'hF;              // B?t 4 server
                else server_en_valid = {NUM_SERVERS{1'b1}};
            end
            
            6'b000111: begin
                if (NUM_SERVERS >= 8) server_en_valid[7:0] = 8'hFF;             // B?t 8 server
                else server_en_valid = {NUM_SERVERS{1'b1}};
            end
            
            6'b001111: begin
                if (NUM_SERVERS >= 16) server_en_valid[15:0] = 16'hFFFF;        // B?t 16 server
                else server_en_valid = {NUM_SERVERS{1'b1}};
            end
            
            6'b011111: server_en_valid = {NUM_SERVERS{1'b1}};                   // B?t t?i ?a (32 ho?c N)
            
            6'b111111: server_en_valid = {NUM_SERVERS{1'b1}};                   // B?t t?t c?
            
            default:   server_en_valid = {NUM_SERVERS{1'b0}};
        endcase
    end
...
//    );
    ...
    packet_filter #(
        .BUS_WIDTH(BUS_WIDTH),
        .FILTER_UDP_IPV4_ONLY(FILTER_UDP_IPV4_ONLY)
    ) u_packet_filter (
        .clk        (clk_core),
        .rst_n      (rst_n),
...
//        .cfg_ip_list    ({32'hc0a8010a, 32'hc0a8010b, 32'hc0a8010c, 32'hc0a8010d}),
        ...
        .health_bitmap_update_valid(health_update_valid),
