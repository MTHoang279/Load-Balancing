...
module shm_parser #(
    ...
    assign src_ip              = rx_user_data[303:272];
    assign src_ip_last_octet   = src_ip[7:0];
    assign server_id_in_range  = (src_ip_last_octet >= 8'd100) &&
                                 (src_ip_last_octet < (8'd100 + NUM_SERVERS));
    assign extracted_server_id = src_ip_last_octet - 8'd100;
    ...
    assign sst_scn_dec_en = rx_user_valid && rx_user_last &&
                            is_user_response && rx_cslb_ready;
    assign sst_server_idx = server_id_in_range ?
                            extracted_server_id :
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            scn_dec_en <= sst_scn_dec_en && server_id_in_range;
            if (server_id_in_range)
                server_idx <= extracted_server_id;
        end
    end
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (rx_user_valid && rx_user_last && is_heartbeat_response && parser_response_ready && server_id_in_range) begin
                parser_response_valid <= 1'b1;
                parser_server_id      <= extracted_server_id;
            end else begin
                parser_response_valid <= 1'b0;
