...
module packet_filter #(
    ...
    (* mark_debug = "true" *) reg [19:0] valid_pkt_count;
    ...
    always @(posedge clk) begin
        if (!rst_n) begin
            ...
            // 1. Clear pulse signals by default each period
            meta_tvalid <= 1'b0;
            done_fil    <= 1'b0;

            // 2. Process output (Data gets out -> clear buffer & check done)
            if (out_fire) begin
                buf_valid <= 1'b0;
                
                // [SỬA LỖI] Báo done_fil và đếm gói khi TỪNG BEAT CUỐI thực sự được truyền đi
                if (buf_last) begin
                    done_fil        <= 1'b1;
                    valid_pkt_count <= valid_pkt_count + 1;
                end
            end
                ...
                case (state)
                    ST_IDLE: begin
                        if (s_tdata[415:400] == ETH_TYPE_IPV4 &&
                            ...
                            // (Đã xóa logic done_fil ở đây để dời lên out_fire)
                        end else begin
                            // Gói KHÔNG hợp lệ -> Drop
                    ...
                    // [SỬA LỖI] Thêm default case để phòng ngừa FSM rơi vào trạng thái ảo (2'b11)
                    default: begin
                        state <= ST_IDLE;
                    end
                endcase
