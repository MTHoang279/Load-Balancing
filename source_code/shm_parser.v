module shm_parser #(
    ...
    localparam integer NUM_SERVERS = (1 << SERVER_ID_WIDTH);
    ...
    wire        user_id_in_range;
    wire        hb_id_in_range;
    wire [SERVER_ID_WIDTH-1:0] extracted_user_id;
    wire [SERVER_ID_WIDTH-1:0] extracted_hb_id;
    ...
    assign user_id_in_range    = (dst_ip_last_octet >= 8'd100) &&
                                (dst_ip_last_octet < (8'd100 + NUM_SERVERS));
    assign hb_id_in_range      = user_id_in_range;
    assign extracted_user_id   = dst_ip_last_octet - 8'd100;
    assign extracted_hb_id     = extracted_user_id;
    ...
    assign sst_scn_dec_en = rx_user_valid && rx_user_last &&
                            is_user_response && rx_cslb_ready && user_id_in_range;
    assign sst_server_idx = extracted_user_id;
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (rx_user_valid && rx_user_last && is_heartbeat_response &&
                parser_response_ready && hb_id_in_range) begin
                parser_response_valid <= 1'b1;
                parser_server_id      <= extracted_hb_id;
            end else begin
                parser_response_valid <= 1'b0;
            end
        end
